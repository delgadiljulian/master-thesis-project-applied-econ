...
1980--2022.
	...
	El análisis empírico se estructura en cinco etapas secuenciales que guían la contrastación de las hipótesis planteadas. En primer lugar, se realiza la descripción y caracterización de trayectorias a través de estadísticos descriptivos y análisis exploratorios de correlación, con el propósito de identificar patrones preliminares en la relación entre dependencia de recursos naturales y transformación estructural. El análisis se realiza sobre los 55 países definidos mediante $DRES\geq20\%$ en 1990--1995 y describe la cobertura efectiva de cada variable sin imputar observaciones faltantes.

	En segundo lugar, se aborda el modelo principal centrado en el Índice de Complejidad Económica ($ECI$), estimando la \autoref{eq:modelo_base} mediante una especificación de datos de panel con efectos fijos por país y por año. Esta especificación incorpora todos los canales teóricos presentados en el diseño metodológico, incluida la interacción constitutiva entre la dependencia extractiva y la calidad institucional ($RENTS_{it}\times INST_{it}$), calculando errores estándar robustos a la estructura de dependencia del panel.

	En tercer lugar, se evalúa el modelo complementario para la diversificación exportadora ($DIVX$), estimando la \autoref{eq:modelo_divx} bajo la misma estructura de efectos fijos bidireccionales y el mismo conjunto de regresores explicativos. En esta ecuación se excluye únicamente el índice de Herfindahl-Hirschman ($HHI$), debido a la identidad contable $DIVX = 1 - HHI$, manteniendo la prevalencia de exportaciones primarias y combustibles en el canal estructural.

	En cuarto lugar, se examina la heterogeneidad por tipo de recurso extractivo reestimando ambas ecuaciones al sustituir la variable agregada $RENTS$ por sus componentes predefinidos de hidrocarburos y minería y carbón. Esta desagregación permite identificar y comparar patrones sectoriales diferenciados sin alterar la selección muestral ni atribuir carácter causal a las diferencias observadas.

	En quinto lugar, se aplica un protocolo econométrico integral de diagnósticos de series de tiempo, estabilidad e inferencia robusta para datos de panel. En esta dimensión, se verifica el orden de integración $I(1)$ de las series mediante pruebas Fisher-ADF y se evalúa la presencia de cointegración multivariada mediante las pruebas de Pedroni (1999), Kao (1999) y Westerlund (2007), garantizando la superconsistencia del estimador TWFE en niveles. Asimismo, se estima un sistema dinámico de impulso-respuesta mediante Proyecciones Locales en Panel (\textcite{Jorda2005}) para horizontes de $h=0$ a $h=5$ años en especificaciones agregadas y desagregadas, evaluando de forma simultánea la estabilidad temporal por submuestras para contrastar el auge del Superciclo (1996--2014) frente al período post-superciclo (2015--2021).

	De forma complementaria, el análisis de robustez e inferencia contrasta la estimación baseline agrupada por país frente a la sensibilidad Jackknife LOO, el \textit{Wild Cluster Bootstrap} (\textcite{CameronGelbachMiller2008}; \textcite{Roodman2019WildBootstrap}), la matriz de covarianza de Driscoll y Kraay (1998) ajustada por $AR(p)$ y correlación espacial, y el esquema PCSE AR(1) de Beck y Katz. Finalmente, se aplica el filtro de Hodrick y Prescott ($\lambda = 6,25$, \textcite{RavnUhlig2002}) sobre el conjunto de 18 variables del modelo para discriminar la tendencia estructural de largo plazo del ciclo estocástico de corto plazo.
	...
	La selección de estimadores en esta investigación se realizó rigurosamente a partir de la pregunta de estudio, la estructura observacional del panel macroeconómico ($N=49$ países, $T=26$ años, con un promedio de 21,3 períodos por país) y los supuestos de identificación causal, descartando deliberadamente la comparación retrospectiva por significancia estadística. A continuación se exponen las razones metodológicas por las cuales diversas técnicas econométricas alternativas no fueron adoptadas como evidencia principal:

	\begin{enumerate}
		\item \textit{Panel Dinámico mediante GMM (Difference-GMM y System-GMM)}: Los estimadores de \textcite{ArellanoBond1991} y \textcite{BlundellBond1998} resultan especialmente aptos para micro-paneles cortos y anchos ($N \to \infty, T \text{ pequeño}$). En macro-paneles con $T \ge 25$, la matriz de instrumentos de GMM crece de forma cuadrática ($T^2$), provocando una \textit{proliferación masiva de instrumentos} (\textcite{Roodman2009TooManyInstruments}). En las pruebas preliminares del modelo, las especificaciones GMM generaron entre 97 y 103 instrumentos para 49--53 países, sobreajustando las variables endógenas y produciendo p-valores del test de Hansen cercanos a uno ($p \in [0,9998; 1,0000]$), lo que invalida su capacidad informativa.
		\item \textit{Variables Instrumentales y 2SLS con Rezagos Internos}: Una estrategia IV/2SLS exige que los instrumentos satisfagan estrictamente la restricción de exclusión (\textcite{AngristImbensRubin1996}; \textcite{AndrewsStockSun2019}). Aunque los ejercicios preliminares con rezagos internos arrojaron estadísticos Kleibergen--Paap de primera etapa elevados (entre 61,8 y 205,1), la precedencia temporal no garantiza exogeneidad frente a shocks persistentes de precios o decisiones de política. Asimismo, un instrumento basado en precios internacionales interactuados afectaría directamente la estructura exportadora y el tipo de cambio real, violando la restricción de exclusión hacia $ECI$ y $DIVX$.
		\item \textit{Modelos de Vectores Autorregresivos en Panel (PVAR)}: La estimación de un sistema PVAR (\textcite{LoveZicchino2006}) exige estimar cada variable en función de los rezagos de todas las demás. Con un vector completo de 18 variables como el del Modelo 3 ($M3$), un PVAR requeriría la estimación de más de 300 parámetros por ecuación, destruyendo los grados de libertad (\textit{maldición de la dimensionalidad}). Por ello, la investigación prioriza \textit{Proyecciones Locales en Panel} (\textcite{Jorda2005}), las cuales estiman ecuaciones dinámicas independientes por horizonte $h$ sin pérdida de grados de libertad.
		\item \textit{Estimadores de Cointegración No Lineal en Panel (PMG, DOLS, FMOLS)}: En presencia de variables $I(1)$ y cointegración en panel (confirmada por las pruebas de Pedroni y Kao con $p < 0,0001$), el estimador OLS con efectos fijos bidireccionales (TWFE) en niveles es superconsistente (\textcite{Kao1999}; \textcite{Pedroni2004}; \textcite{PhillipsMoon1999}). Por ello, estimadores como Pooled Mean Group (PMG) o Dynamic OLS (DOLS) no aportan consistencia adicional y restan eficiencia ante la inclusión de 15 regresores.
		\item \textit{Regresión Discontinua (RDD) y Propensity Score Matching (PSM)}: RDD requiere una variable de selección continua con un umbral institucional arbitrario de elegibilidad (\textit{cut-off}), inexistente en la dotación de recursos naturales. Asimismo, el emparejamiento por PSM está diseñado para datos de corte transversal y no permite controlar por la heterogeneidad inobservable de país invariante en el tiempo ($\alpha_i$).
		\item \textit{Modelos Tobit y Selección Muestral de Heckman}: Los modelos de censura y selección muestral (\textcite{Heckman1979}) corrigen el sesgo cuando la variable dependiente sólo se observa para una submuestra no aleatoria. En este estudio, las variables dependientes ($ECI$ y $DIVX$) constituyen series continuas, completas e ininterrumpidas para los 49 países durante el período 1996--2021, descartando cualquier sesgo de truncamiento.
	\end{enumerate}
