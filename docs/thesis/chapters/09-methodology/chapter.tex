...
% =====================================
	...
	econométrica se define como 1996--2021, dado que los cuatro componentes
	requeridos para construir $RENTS$ se publican hasta 2021. El intervalo 1990--1995 se utiliza
	exclusivamente como período base para clasificar la dependencia externa de
	recursos naturales mediante el indicador $DRES$.

	En este marco, la estrategia metodológica combina análisis descriptivo y
	dos estimaciones en datos de panel con efectos fijos. La especificación
	...
	datos del período base, esta regla produce una muestra fija de 55 países.
...
Para mantener una definición homogénea, todas las observaciones se extraen en
una misma captura de la API y no se combinan con la antigua tabla de rankings
ni con indicadores de otros proveedores.
...
1980--2022.
	...
	\begin{longtable}{|
		...
		& Índice de capital humano \texttt{hc}, construido a partir de años promedio de escolaridad y retornos a la educación; proxy de capacidades productivas acumuladas. No se interpreta como años de escolaridad.
		& Penn World Table 11.0 \\
		\hline
		...
		& GOVCONS
		& Control fiscal
		& Gasto de consumo final del gobierno general como porcentaje del PIB (\texttt{NE.CON.GOVT.ZS}); controla por el tamaño relativo del consumo público y no se interpreta como una medida directa de capacidad ni de prociclicidad fiscal.
		& World Development Indicators (Banco Mundial) \\
		\hline
		...
		& Crédito doméstico al sector privado como porcentaje del PIB (\texttt{FS.AST.PRVT.GD.ZS}); proxy de la profundidad del crédito privado, no de su acceso, eficiencia o calidad.
		& World Development Indicators (Banco Mundial) \\
		\hline

	\end{longtable}

	Las rentas extractivas se construirán únicamente cuando estén observados sus
	cuatro componentes:
	\[
		RENTS_{it}
		=
		...
		MINERALRENT_{it}.
	\]
	Los valores faltantes no se sustituirán por cero. Los cuatro indicadores
	...
	El índice institucional se calculará como el promedio simple de los tres
	...
	$GE$ efectividad gubernamental. El índice se construirá únicamente cuando
	los tres componentes estén disponibles; no se interpolarán los vacíos
	estructurales de 1997, 1999 y 2001.

	Las medidas de rentas por habitante se construirán de forma homogénea:
	\[
		KPC_{it}
		=
		\frac{
	...
	población total. No se imputarán componentes faltantes.

	La medida de volatilidad se construirá a partir del
	\textit{Commodity Net Export Price Index} del FMI
	(\texttt{CEMPI\_CTOTNX\_TT}). Para cada país, primero se calculará la
	...
	\[
		...
		\right].
	\]
	A continuación, la volatilidad se definirá como la desviación estándar
	...
	\[
		...
		\right).
	\]
	La variable utilizará exclusivamente el índice con ponderaciones fijas
	...
	La medida de capital humano será el índice \texttt{hc} de Penn
	World Table 11.0. Este índice combina los años promedio de escolaridad con
	una función de retornos a la educación y se utilizará en niveles. En
	consecuencia, $HUMCAP_{it}$ representa un índice de capacidades acumuladas y
	no los años de escolaridad observados directamente.
	...
	El tipo de cambio real y el control de desarrollo se construirán,
	...
	especializadas. En particular, la evidencia empírica procederá de fuentes
		comparables a escala internacional: los \textit{World Development
		Indicators} y \textit{Worldwide Governance Indicators} del Banco
		Mundial, el \textit{Atlas of Economic Complexity} del Harvard Growth
		Lab, el Fondo Monetario Internacional y Penn World Table, según la
		variable considerada y tal como se resume en
	...
	transparencia metodológica y disponibilidad para el horizonte general de recopilación 1980--2022 y, especialmente, para el período principal de estimación 1996--2021. El índice CTOT constituye una excepción histórica: su fuente ofrece observaciones desde 1962, aunque en esta tesis se utilizará solamente la información necesaria para construir $VOL$ durante el período principal.
	...
	de identificadores de país y período, la homologación de unidades de
		medida y la verificación de consistencia interna de cada fuente.
		Asimismo, algunas variables serán construidas a partir de
	...
	controles de consistencia temporal y de rango, revisión de observaciones
	duplicadas, documentación de valores atípicos y trazabilidad completa entre
	las series originales y las transformaciones definidas. No se sustituirán
	automáticamente los datos faltantes por ceros ni se incorporarán
	transformaciones distintas de las establecidas en esta sección. Dado que la disponibilidad de información no es homogénea
	...
	Estas dos ecuaciones agotan la estrategia econométrica de la tesis. Cada
	variable explicativa tiene una única definición operativa y una única fuente
	activa. No se estiman proxies alternativas, submuestras por otros umbrales,
	interacciones temporales ni modelos adicionales. La especificación con
	$DIVX$ es el único modelo complementario al modelo principal con $ECI$.
	...
	dos ecuaciones se estimarán con efectos fijos por país y por año; no se
	estimarán modelos con efectos aleatorios, términos rezagados ni
	...
	El análisis empírico se estructura en tres etapas: una caracterización
	descriptiva de la muestra fija, la estimación principal con $ECI$ y la
	estimación complementaria con $DIVX$. Esta secuencia mantiene el análisis
	centrado en las asociaciones condicionales entre rentas extractivas,
	instituciones, capacidades productivas y transformación estructural.
	...
	En conjunto, esta estrategia permite contrastar las hipótesis del trabajo
	sin ampliar innecesariamente el número de modelos. Los resultados se
	interpretarán dentro de las limitaciones propias de un diseño
