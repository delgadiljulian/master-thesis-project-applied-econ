...
1980--2022.
	...
	\[
		RENTS^{OIL+GAS}_{it}
		=
		OILRENT_{it}+GASRENT_{it},
		\qquad
		RENTS^{MINING}_{it}
		=
		COALRENT_{it}+MINERALRENT_{it}.
	\]
	...
	\subsection{Cobertura efectiva del panel maestro}
	\label{subsec:cobertura-panel-maestro}

	La cobertura individual es elevada: todas las variables del modelo superan
	el 80\% de las 1.430 observaciones potenciales y siete de ellas tienen
	cobertura completa. La \autoref{tab:cobertura-panel-maestro} presenta el
	diagnóstico reproducible obtenido después de integrar las salidas
	procesadas.

	\begin{table}[htbp]
		\centering
		\caption{Cobertura de las variables en el panel maestro, 1996--2021}
		\label{tab:cobertura-panel-maestro}
		\begin{tabular}{lrr}
			\toprule
			Variable & Observaciones & Cobertura (\%) \\
			\midrule
			$ECI$, $PEXP$, $FEXP$, $INNOV$, $GOVCONS$, $HHI$, $DIVX$ & 1.430 & 100,0 \\
			$VOL$, $HUMCAP$ & 1.403 & 98,1 \\
			$OILPC$ & 1.397 & 97,7 \\
			$COALPC$ & 1.384 & 96,8 \\
			$NET$ & 1.379 & 96,4 \\
			$LOG\_GDPPC$ & 1.378 & 96,4 \\
			$GASPC$ & 1.365 & 95,5 \\
			$RENTS$, $RER$ & 1.352 & 94,5 \\
			$FIN$ & 1.328 & 92,9 \\
			$INST$ & 1.257 & 87,9 \\
			$RENTS\times INST$ & 1.197 & 83,7 \\
			\bottomrule
		\end{tabular}
		\begin{minipage}{0.94\textwidth}
			\footnotesize
			\textit{Nota:} La cobertura se calcula sobre 55 países y 26 años.
			Los WGI no publicaron observaciones en 1997, 1999 y 2001; estos vacíos
			estructurales se conservan sin interpolación. Dentro de los 23 años
			publicados por WGI, la interacción $RENTS\times INST$ alcanza una
			cobertura de 94,6\%.
		\end{minipage}
	\end{table}

	La coincidencia simultánea de todas las variables requeridas por las
	especificaciones econométricas corresponde a 1.044 observaciones de 49 países
	(73,0\% de la grilla maestra). Esta reducción no contradice la cobertura
	individual superior al 80\%: resulta de intersectar faltantes que no siempre
	coinciden en los mismos países y años. Por tanto, cada estimación reportará
	su número efectivo de observaciones y países, sin imputar información ni
	alterar la selección original de 55 economías.
