...
\begin{document}
	...
	La \autoref{tab:fuentes_datos} resume los indicadores definidos para la construcción del panel empírico, sus fuentes internacionales y la cobertura potencial de los insumos conservados en \texttt{data/raw}. Cada indicador tiene una única definición activa; la muestra efectiva se determinará posteriormente a partir de la intersección de datos disponibles, sin imputar los faltantes.
	...
	\begin{footnotesize}
		\begin{longtable}{
			...
			& Capital humano
			& Índice de capital humano \texttt{hc} (HUMCAP)
			...
			& Consumo final del gobierno general (\% del PIB; \texttt{NE.CON.GOVT.ZS}) (GOVCONS)
			& World Bank (WDI)
			& 1980--2022 \\

			Canal financiero
			& Profundidad financiera
			& Crédito doméstico al sector privado (\% PIB; FS.AST.PRVT.GD.ZS) (FIN)
			& World Bank (WDI)
			& 1980--2022 \\

			\hline
			\multicolumn{5}{p{14.5cm}}{\textit{Nota: Los períodos consignan la cobertura potencial de los insumos raw, no un panel balanceado. El período econométrico efectivo es 1996--2021 porque los cuatro componentes de $RENTS$ terminan en 2021. $DRES$ se utiliza únicamente para seleccionar los 55 países con el umbral de 20\%. $HHI$ se incluye solo en el modelo con $ECI$ y se excluye cuando $DIVX=1-HHI$ es el resultado complementario.}} \\
		\end{longtable}
	\end{footnotesize}
