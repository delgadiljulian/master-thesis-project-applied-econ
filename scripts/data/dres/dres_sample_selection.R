# Construir DRES y seleccionar las muestras de países dependientes de recursos del subsuelo.
#
# Entradas: exportaciones Atlas SITC Rev. 2, catálogo de productos y población WDI.
# Salidas: panel país-año, promedio 1990-1995 y muestras DRES de 20 %, 30 % y 40 %.
...
helper_path <- c(
  ...
  # Buscar el helper cuando la consola está ubicada en scripts/data/dres/.
...
# Localizar la población WDI que se conservará únicamente como diagnóstico de robustez.
...
dres_country_profile$eligible_for_dres <-
  ...
  dres_country_profile$has_complete_base_merchandise
# Conservar el antiguo umbral poblacional exclusivamente como indicador de robustez.
dres_country_profile$population_ge_1m_robustness <-
  !is.na(dres_country_profile$population_base_mean) &
  dres_country_profile$population_base_mean >= 1000000
# Conservar el antiguo umbral comercial exclusivamente como indicador de robustez.
dres_country_profile$exports_gt_1bn_robustness <-
...
dres_country_profile$sample_dres20 <-
  ...
  dres_country_profile$dres_base_mean >= sample_thresholds[1]
# Crear la primera muestra estricta con un promedio DRES de al menos 30 %.
dres_country_profile$sample_dres30 <-
  dres_country_profile$eligible_for_dres &
  !is.na(dres_country_profile$dres_base_mean) &
  dres_country_profile$dres_base_mean >= sample_thresholds[2]
# Crear la segunda muestra estricta con un promedio DRES de al menos 40 %.
dres_country_profile$sample_dres40 <-
  dres_country_profile$eligible_for_dres &
  !is.na(dres_country_profile$dres_base_mean) &
  dres_country_profile$dres_base_mean >= sample_thresholds[3]
...
# Confirmar que la muestra de 40 % esté contenida completamente en la de 30 %.
if (any(dres_country_profile$sample_dres40 & !dres_country_profile$sample_dres30)) {
  stop("La muestra DRES 40 no está contenida en la muestra DRES 30.")
}
# Confirmar que la muestra de 30 % esté contenida completamente en la de 20 %.
if (any(dres_country_profile$sample_dres30 & !dres_country_profile$sample_dres20)) {
  stop("La muestra DRES 30 no está contenida en la muestra DRES 20.")
}
# Impedir que una economía excluida territorialmente aparezca en cualquier muestra.
if (any(
  !dres_country_profile$is_country_scope &
    (dres_country_profile$sample_dres20 |
      dres_country_profile$sample_dres30 |
...
sample_dres20 <- build_sample_table("sample_dres20", sample_thresholds[1])
# Preparar la lista correspondiente al umbral intermedio de 30 %.
sample_dres30 <- build_sample_table("sample_dres30", sample_thresholds[2])
# Preparar la lista correspondiente al umbral estricto de 40 %.
sample_dres40 <- build_sample_table("sample_dres40", sample_thresholds[3])

# Reunir los tres conteos obtenidos para compararlos con la auditoría validada.
actual_sample_counts <- c(nrow(sample_dres20), nrow(sample_dres30), nrow(sample_dres40))
# Detener el script si la fuente o las reglas producen conteos distintos sin revisión previa.
if (!identical(actual_sample_counts, expected_sample_counts)) {
  stop(
    "Los conteos DRES cambiaron. Esperados: ",
    paste(expected_sample_counts, collapse = ", "),
    "; obtenidos: ",
    paste(actual_sample_counts, collapse = ", "),
...
dres_summary <- data.frame(
  ...
  metric = c(
    ...
    "sample_dres30_countries",
    "sample_dres40_countries",
...
# Guardar el perfil completo con cobertura, banderas y diagnósticos de robustez.
write.csv(
...
# Guardar la lista intermedia de países dependientes según el umbral de 30 %.
write.csv(
  sample_dres30,
  file.path(processed_path, "dres_sample_30.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
# Guardar la lista estricta de países dependientes según el umbral de 40 %.
write.csv(
  sample_dres40,
  file.path(processed_path, "dres_sample_40.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
# Mostrar el tamaño de la muestra principal de 20 %.
message("Muestra DRES 20 %: ", nrow(sample_dres20), " países.")
# Mostrar el tamaño de la muestra intermedia de 30 %.
message("Muestra DRES 30 %: ", nrow(sample_dres30), " países.")
# Mostrar el tamaño de la muestra estricta de 40 %.
message("Muestra DRES 40 %: ", nrow(sample_dres40), " países.")
