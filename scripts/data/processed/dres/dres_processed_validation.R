# CAPA: PROCESSED
# VARIABLE: DRES
# ENTRADAS: data/raw/atlas/, data/raw/dres/ y data/processed/dres/
# SALIDAS: data/processed/dres/validation/
# Auditar la construcción procesada de DRES mediante controles de calidad.
#
# Entradas: comercio Atlas SITC Rev. 2, catálogos Atlas, perfil DRES y validación WDI.
# Salidas: perfiles de validación, cambios de clasificación y controles de calidad.
...
helper_path <- c(
  ...
  # Buscar el helper cuando R se ejecuta desde scripts/data/processed/dres/.
...
# Definir el único umbral utilizado por las dos especificaciones de la tesis.
sample_thresholds <- 0.20
...
# Construir la carpeta específica que recibirá las salidas de validación.
output_path <- file.path(project_path, "data", "processed", "dres", "validation")
...
audit_country_file <- function(country_file) {
  ...
  for (year_value in base_years) {
    ...
    # Conservar el denominador de mercancías con los signos originales para validación.
...
# Construir una fila de validación por economía con las definiciones auditadas.
robustness_profile <- do.call(rbind, lapply(country_groups, function(country_data) {
  ...
  # Devolver una fila nacional con todas las métricas de validación.
  data.frame(
...
# Crear banderas de pertenencia para cada definición con el umbral único.
...
for (variant_index in seq_along(variant_columns)) {
  ...
  # Crear una fila con el conteo correspondiente al umbral de la tesis.
...
write.csv(
  annual_audit,
  file.path(output_path, "dres_validation_country_year.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
# Guardar el perfil nacional con métricas y banderas de validación.
write.csv(
  robustness_profile,
  file.path(output_path, "dres_validation_country_profile.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
write.csv(
  sample_counts,
  file.path(output_path, "dres_validation_sample_counts.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
write.csv(
  membership_changes,
  file.path(output_path, "dres_validation_membership_changes.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
write.csv(
  scope_audit,
  file.path(output_path, "dres_product_scope_validation.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
write.csv(
  quality_summary,
  file.path(output_path, "dres_validation_quality_summary.csv"),
  row.names = FALSE,
  na = "",
  fileEncoding = "UTF-8"
)
...
message("Auditoría de validación de DRES terminada.")
