...
helper_path <- c(
  ...
  # Buscar desde scripts/data/world_bank_wdi/.
...
# Crear el catálogo de indicadores estrictamente necesarios o útiles para validación.
indicator_catalog <- data.frame(
  ...
  purpose = c(
    ...
    "Conservar una alternativa nominal para validación",
    "Construir LOG_GDPPC en PPP constante",
    "Construir FIN",
    "Construir RER cuando exista la serie efectiva",
    "Conservar el proxy fiscal utilizado actualmente",
    "Evaluar capacidad fiscal mediante ingresos tributarios",
    "Construir INNOV principal mediante artículos científicos per cápita",
    "Construir patentes per cápita",
    "Evaluar I+D como alternativa de INNOV",
    "Construir NET",
    "Conservar la alternativa WDI de HUMCAP"
  ),
...
download_wdi_indicator <- function(indicator_code, variable_name, years, zip_dir) {
  ...
  # Informar en la consola qué indicador está comenzando a descargarse.
  message("Descargando ", indicator_code, "...")
  # Descargar el ZIP original desde el servicio oficial del Banco Mundial.
  download.file(
    url = source_url,
    destfile = zip_file,
    mode = "wb",
