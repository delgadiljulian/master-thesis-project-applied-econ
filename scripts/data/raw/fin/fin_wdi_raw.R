# CAPA: RAW
# VARIABLE: FIN
# SALIDAS: data/raw/fin/
# Proposito: preparar el indicador raw de profundidad financiera FIN.
...
helper_path <- c(
  ...
  # Buscar desde scripts/data/raw/fin/.
...
# Definir el indicador oficial y los identificadores.
value_column <- "domestic_credit_private_pct_gdp"
required_columns <- c(
  "country_iso3_code",
  "country",
  "year",
...
# Conservar el indicador publicado sin renombrarlo como FIN ni transformarlo.
fin_raw <- wdi_inputs[, required_columns]
fin_raw$country_iso3_code <- toupper(trimws(fin_raw$country_iso3_code))
fin_raw$year <- as.integer(fin_raw$year)
fin_raw <- fin_raw[order(fin_raw$country_iso3_code, fin_raw$year), ]
row.names(fin_raw) <- NULL
...
if (anyDuplicated(fin_raw[c("country_iso3_code", "year")]) > 0L) {
  stop("El insumo raw de FIN contiene llaves pais-anio duplicadas.")
}
if (any(!grepl("^[A-Z]{3}$", fin_raw$country_iso3_code))) {
  stop("El insumo raw de FIN contiene codigos de pais invalidos.")
}
if (any(!fin_raw$year %in% 1980:2022)) {
  stop("El insumo raw de FIN contiene anios por fuera de 1980-2022.")
}
observed_raw_values <- fin_raw[[value_column]][
  !is.na(fin_raw[[value_column]])
]
if (any(!is.finite(observed_raw_values))) {
  stop("El indicador financiero contiene valores no finitos.")
}
if (any(fin_raw[[value_column]] < 0, na.rm = TRUE)) {
  stop("El indicador financiero contiene valores negativos.")
}

# No imponer un maximo de 100: el credito privado puede superar el PIB anual.
missing_sample_countries <- setdiff(
  dres_sample$country_iso3_code,
  fin_raw$country_iso3_code
)
...
if (anyDuplicated(sample_grid[c("country_iso3_code", "year")]) > 0L) {
  stop("La cuadricula DRES de FIN contiene llaves duplicadas.")
}
...
country_coverage_rows <- lapply(
  ...
  function(country_code) {
    ...
    data.frame(
      ...
      observations_above_100_percent = sum(observed_values > 100),
...
# Resumir cobertura y rango sin recortar los valores superiores a 100.
...
csv_file <- file.path(output_path, "fin_wdi_input_1980_2022.csv")
dta_file <- file.path(output_path, "fin_wdi_input_1980_2022.dta")
summary_file <- file.path(
  output_path,
  "fin_raw_coverage_summary_dres20.csv"
)
country_coverage_file <- file.path(
  output_path,
  "fin_raw_country_coverage_dres20.csv"
)
...
if (requireNamespace("haven", quietly = TRUE)) {
  haven::write_dta(fin_raw, dta_file, version = 14)
} else {
  foreign::write.dta(fin_raw, dta_file, version = 12)
}
write.csv(
  coverage_summary,
  summary_file,
