...
//
// Archivo: 02_econometric_models.do (Versión Codex)
// Contenido: Secciones 5 a 8 del análisis econométrico
// Requisito: ejecutar primero 01_data_preparation_diagnostics.do
...
* Este archivo puede ejecutarse en una sesión nueva después de completar el
* archivo 01. La numeración econométrica continúa directamente en la sección 5.
version 17.0
clear all
cls
macro drop _all
...
* Examinar primero el directorio actual y hasta cinco niveles superiores.
...
* cinco niveles. Esto permite ejecutarlo desde su carpeta o desde logs/batch
* sin depender de una ruta absoluta.
local project_candidate "`project_current'"
forvalues search_level = 0/5 {
    if "$PROJECT_ROOT" == "" {
...
// A.3. Definir las entradas, salidas y el log del archivo 02
...
global OUTPUT_ROOT ///
    "$PROJECT_ROOT/outputs/econometrics/stata-peer-2"
...
* Abrir un registro de texto exclusivo para las secciones 5 a 8. replace
* garantiza que el log corresponda únicamente a la ejecución más reciente.
log using "$OUTPUT_LOGS/02_econometric_models.log", ///
...
// 5.1. Cargar y verificar la muestra del modelo principal
...
// 5.2. Definir la ecuación ECI y la estrategia de inferencia
...
// 5.3. Estimar el modelo principal mediante xtreg
...
// 5.4. Exportar los coeficientes y la incertidumbre del modelo
...
// 5.5. Exportar el resumen general de la estimación
...
// 5.6. Realizar las pruebas conjuntas previstas por canal
...
// 5.7. Verificar los coeficientes mediante reghdfe
...
// 5.8. Crear una tabla LaTeX provisional para revisión
...
// 6.1. Cargar y verificar la muestra del modelo complementario

* Abrir nuevamente la base analítica completa. Este paso evita que una
* transformación temporal de la sección 5 pueda afectar el modelo DIVX.
use "$OUTPUT_SAMPLE/master_panel_estimation_sample.dta", clear
...
// 6.2. Definir la ecuación DIVX y excluir HHI
...
// 6.3. Estimar el modelo complementario mediante xtreg
...
// 6.4. Exportar los coeficientes y la incertidumbre del modelo
...
// 6.5. Exportar el resumen general de la estimación
...
// 6.6. Realizar las pruebas conjuntas previstas por canal
...
// 6.7. Verificar los coeficientes mediante reghdfe
...
// 6.8. Crear una tabla LaTeX provisional para revisión
...
// 7.1. Recuperar la base analítica y definir valores observados de INST
...
// 7.2. Calcular los efectos marginales de RENTS según INST
...
// 7.3. Resumir la prueba de igualdad entre petróleo, gas y carbón
...
// 7.4. Evaluar la forma funcional de las rentas per cápita
...
// 7.5. Evaluar sensibilidad a las observaciones previamente señaladas
...
// 7.6. Comprobar dependencia respecto de un solo país
...
// 7.7. Aplicar wild cluster bootstrap como inferencia alternativa
...
// 7.8. Cerrar la sección sin sustituir los modelos principales
...
// 8.1. Confirmar los modelos y resultados que alimentarán el paquete final
...
local prerequisite_files ///
    outputs/econometrics/stata-peer-2/03_eci/eci_twfe_coefficients.csv ///
    outputs/econometrics/stata-peer-2/03_eci/eci_twfe_model_summary.csv ///
    outputs/econometrics/stata-peer-2/03_eci/eci_twfe_joint_tests.csv ///
    outputs/econometrics/stata-peer-2/04_divx/divx_twfe_coefficients.csv ///
    outputs/econometrics/stata-peer-2/04_divx/divx_twfe_model_summary.csv ///
    outputs/econometrics/stata-peer-2/04_divx/divx_twfe_joint_tests.csv ///
    outputs/econometrics/stata-peer-2/05_stability/rents_marginal_effects_by_inst.csv ///
    outputs/econometrics/stata-peer-2/05_stability/resource_coefficient_equality.csv ///
    outputs/econometrics/stata-peer-2/05_stability/per_capita_transformation_sensitivity.csv ///
    outputs/econometrics/stata-peer-2/05_stability/influential_observation_sensitivity.csv ///
    outputs/econometrics/stata-peer-2/05_stability/leave_one_country_out_summary.csv ///
    outputs/econometrics/stata-peer-2/05_stability/wild_cluster_bootstrap.csv
...
// 8.2. Exportar la tabla econométrica principal ECI-DIVX
...
// 8.3. Consolidar coeficientes, resúmenes y pruebas conjuntas
...
// 8.4. Clasificar la evidencia sin convertir asociaciones en causalidad
...
// 8.5. Reunir las sensibilidades y figuras finales
...
// 8.6. Crear el índice reproducible de resultados
...
file write `results_index' ///
    "Generado por 02_econometric_models.do el `c(current_date)'." _n _n
file write `results_index' ///
...
// 8.7. Crear el manifiesto y validar el paquete final
...
foreach final_file of local final_required_files {
    capture confirm file ///
        "outputs/econometrics/stata-peer-2/06_final/`final_file'"
    if _rc {
        display as error ///
...
* Cerrar el registro después de completar y validar las secciones 5 a 8.
display as result "Archivo 02 finalizado sin errores."
