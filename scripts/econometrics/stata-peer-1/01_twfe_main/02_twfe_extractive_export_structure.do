...
//
// Archivo: 02_twfe_extractive_export_structure.do (Versión Peer-1)
// Contenido: Modelo 1 (M1) — Estructura extractiva y especialización exportadora
...
capture shell mkdir "$PROJECT_ROOT\outputs\econometrics\stata-peer-1\01_twfe_main\08_extractive_export_structure"
...
* Abrir el registro de ejecución de la Parte 2.
log using "$OUTPUT_LOGS/02_twfe_extractive_export_structure.log", text replace name(m1_log)
...
// *****************************************************************************
// 3. Estimación del Modelo 1 (M1) — Complejidad Económica (ECI)
// *****************************************************************************

* Estimar la regresión TWFE de ECI sobre el bloque de estructura extractiva y exportadora.
reghdfe eci rents inst rents_x_inst log_oilpc log_gaspc log_coalpc hhi pexp fexp log_gdppc if sample_eci == 1, absorb(country_id year) vce(cluster country_id)

* Guardar el modelo estimado en la memoria interna de Stata.
estimates store M1_ECI

* Guardar el objeto de estimación en disco para reproducibilidad futura.
estimates save "$OUTPUT_M1/m1_eci.ster", replace
estimates save "$OUTPUT_ECI/m1_eci.ster", replace

* Evaluar la significancia conjunta del bloque de abundancia física per cápita.
quietly test log_oilpc log_gaspc log_coalpc
local f_phys_eci = r(F)
local p_phys_eci = r(p)

* Evaluar la significancia conjunta del bloque de estructura exportadora.
quietly test hhi pexp fexp
local f_struct_eci = r(F)
local p_struct_eci = r(p)
...
* Crear tabla borrador temporal para exportar los efectos marginales de ECI.
...
// *****************************************************************************
// 4. Estimación del Modelo 1 (M1) — Diversificación Exportadora (DIVX)
// *****************************************************************************

* Estimar la regresión TWFE de DIVX sobre el bloque de estructura extractiva (excluyendo HHI).
reghdfe divx rents inst rents_x_inst log_oilpc log_gaspc log_coalpc pexp fexp log_gdppc if sample_divx == 1, absorb(country_id year) vce(cluster country_id)

* Guardar el modelo estimado en la memoria interna de Stata.
estimates store M1_DIVX

* Guardar el objeto de estimación en disco para reproducibilidad futura.
estimates save "$OUTPUT_M1/m1_divx.ster", replace
estimates save "$OUTPUT_DIVX/m1_divx.ster", replace

* Evaluar la significancia conjunta del bloque de abundancia física per cápita.
quietly test log_oilpc log_gaspc log_coalpc
local f_phys_divx = r(F)
local p_phys_divx = r(p)

* Evaluar la significancia conjunta del bloque de estructura exportadora.
quietly test pexp fexp
local f_struct_divx = r(F)
local p_struct_divx = r(p)
...
* Crear tabla borrador temporal para exportar los efectos marginales de DIVX.
...
* Consolidar y exportar la tabla de efectos marginales de RENTS a CSV.
preserve
    use "`me_file_eci'", clear
    append using "`me_file_divx'"
    format inst_val dydx_rents std_err t_stat p_val ci_low ci_high %12.4f
    export delimited using "$OUTPUT_M1/m1_marginal_effects_rents.csv", replace
    list, sepby(outcome) noobs abbreviate(20)
restore

* Crear y exportar la tabla resumen de pruebas F de significancia conjunta a CSV.
tempname ftest_post
tempfile ftest_report
postfile `ftest_post' str8 outcome str32 block double f_stat double p_val using "`ftest_report'", replace

post `ftest_post' ("ECI")  ("Abundancia física (OIL, GAS, COAL)") (`f_phys_eci')   (`p_phys_eci')
post `ftest_post' ("ECI")  ("Estructura exportadora (HHI, PEXP, FEXP)") (`f_struct_eci') (`p_struct_eci')
post `ftest_post' ("ECI")  ("Mecanismo de rentas (RENTS + RENTSxINST)") (`f_rents_eci')  (`p_rents_eci')

post `ftest_post' ("DIVX") ("Abundancia física (OIL, GAS, COAL)") (`f_phys_divx')   (`p_phys_divx')
post `ftest_post' ("DIVX") ("Estructura exportadora (PEXP, FEXP)") (`f_struct_divx') (`p_struct_divx')
...
preserve
    ...
    export delimited using "$OUTPUT_M1/m1_joint_tests.csv", replace
    list, sepby(outcome) noobs abbreviate(30)
restore

* Exportar la tabla comparativa de regresiones M1 (ECI vs DIVX) en formato LaTeX y texto.
esttab M1_ECI M1_DIVX using "$OUTPUT_M1/m1_main_table.tex", replace ///
    ...
    mtitles("ECI" "DIVX") title("Modelo 1 (M1): Estructura Extractiva y Especialización Exportadora") ///
    booktabs label alignment(c)

* Informar en consola el cierre exitoso del archivo 02.
display as result "---------------------------------------------------------"
display as result "Archivo 02 finalizado: secciones 5 y 6 completadas sin errores."
display as result "Resultados guardados en: $OUTPUT_M1"
display as result "---------------------------------------------------------"

* Cerrar el registro de ejecución de la Parte 2.
log close m1_log
