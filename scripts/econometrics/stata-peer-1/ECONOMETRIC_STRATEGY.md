...
| Complementario | Within-between de Mundlak | Separar variación dentro y entre países | Descomposición descriptiva |
...
- diferencias entre rentas de hidrocarburos y rentas mineras.
...
Para ECI y DIVX se utilizará la misma secuencia predefinida:

1. **Focal:** RENTS, INST y RENTS × INST, con efectos fijos de país y año.
2. **Bloques:** añadir por separado abundancia física, estructura exportadora,
   macroeconomía, capacidades y controles económicos.
3. **Completa:** incorporar todos los bloques sobre la misma muestra.
4. **Desagregada:** sustituir RENTS por hidrocarburos y minería/carbón.

La columna focal será la referencia para la asociación total condicionada por
efectos fijos. La columna completa se interpretará como asociación directa
condicionada, porque varios controles pueden ser canales afectados por las
rentas. La reducción de un coeficiente entre columnas no demuestra mediación.
...
1. Preservar los resultados TWFE ya validados.
2. Añadir la secuencia focal–bloques–completa sobre muestras disponibles y
   comunes.
3. Ejecutar el módulo temporal con decisiones de rezago predefinidas.
4. Ejecutar la descomposición within-between de manera parsimoniosa.
5. Comparar signos, magnitudes, intervalos y muestras, sin votar resultados por
   número de métodos significativos.
6. Actualizar el TFM únicamente después de validar los dos módulos nuevos.

## 9. Criterio final para el TFM

El TFM debe sostener sus conclusiones en TWFE. Los módulos temporal y
within-between amplían la lectura, pero no convierten el estudio en causal. Si
alguno de ellos resulta inestable o no identificable con la muestra disponible,
se documentará y se excluirá sin reemplazarlo por GMM o IV.
